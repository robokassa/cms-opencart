...
class Robokassa extends \Opencart\System\Engine\Controller
{
    ...
    public function index(): void
    {
        ...
        $data['payment_robokassa_password_3'] = $this->request->post['payment_robokassa_password_3'] ?? $this->config->get('payment_robokassa_password_3');
        ...
        $data['payment_robokassa_refund_cron_copy_url'] = $this->url->link('extension/robokassa/payment/robokassa|copyRefundCronUrl', 'user_token=' . $this->session->data['user_token'], true);
        $data['payment_robokassa_refund_cron_regenerate_url'] = $this->url->link('extension/robokassa/payment/robokassa|regenerateRefundCronToken', 'user_token=' . $this->session->data['user_token'], true);
    ...
    public function refund(): void
    {
        ...
        if ($this->request->server['REQUEST_METHOD'] === 'POST') {
            ...
            $this->response->redirect($this->url->link('extension/robokassa/payment/robokassa|refund', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $order_id, true));
            return;
        }
        ...
        $data['breadcrumbs'] = array(
            ...
            array('text' => $data['heading_title'], 'href' => $this->url->link('extension/robokassa/payment/robokassa|refund', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $order_id, true))
        );
        $data['action'] = $this->url->link('extension/robokassa/payment/robokassa|refund', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $order_id, true);
        $data['back'] = $this->url->link('sale/order|info', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $order_id, true);
        $data['check_url'] = str_replace('&amp;', '&', $this->url->link('extension/robokassa/payment/robokassa|checkRefund', 'user_token=' . $this->session->data['user_token'], true));
        $data['order_id'] = $order_id;
        $data['order_info'] = $order_info;
        $data['currency_code'] = $order_info['currency_code'];
        $data['order_total'] = round((float)$order_info['total'], 2);
        $data['reserved_total'] = round($this->model_extension_robokassa_payment_robokassa->getReservedRefundTotal($order_id), 2);
        ...
        $refundable = $this->model_extension_robokassa_payment_robokassa->getRefundableData($order_id);
        $data['products'] = $refundable['products'];
        $data['shipping'] = $refundable['shipping'];
        $data['refunds'] = $this->model_extension_robokassa_payment_robokassa->getRefunds($order_id);
        ...
        $this->response->setOutput($this->load->view('extension/robokassa/payment/robokassa_refund', $data));
    }

    public function checkRefund(): void
    {
        $this->load->language('extension/robokassa/payment/robokassa');
        $this->load->model('extension/robokassa/payment/robokassa');
        $this->model_extension_robokassa_payment_robokassa->installMarkingTables();
        ...
        if (!$this->user->hasPermission('modify', 'extension/robokassa/payment/robokassa')) {
            ...
            $refund = $this->model_extension_robokassa_payment_robokassa->getRefund($refund_id);
            ...
            if (!$refund || !$refund['request_id']) {
                ...
                if (!$result['success']) {
                    $this->model_extension_robokassa_payment_robokassa->updateRefundState($refund_id, $refund['status'] === 'unknown' ? 'unknown' : 'processing', $result['raw'], $result['error']);
                    ...
                    if (!in_array($status, array('processing', 'finished', 'canceled'), true)) {
                        ...
                        $state_updated = $this->model_extension_robokassa_payment_robokassa->updateRefundState($refund_id, $status, $result['raw']);

                        if ($state_updated && $status !== $refund['status'] && $status === 'finished') {
                            $this->model_extension_robokassa_payment_robokassa->recordFinishedRefund(
                                $refund['order_id'],
                            ...
                            $this->model_extension_robokassa_payment_robokassa->addOrderNote($refund['order_id'], sprintf($this->language->get('note_refund_canceled'), $refund['request_id']));
                        }
    ...
    private function submitRefund(array $order_info)
    {
        ...
        $remaining = max(0, round((float)$order_info['total'] - $this->model_extension_robokassa_payment_robokassa->getReservedRefundTotal($order_id), 2));
        ...
        if ($receipt_mode) {
            try {
                $built = $this->model_extension_robokassa_payment_robokassa->buildRefundInvoiceItems(
                    $order_id,
        ...
        $reason = isset($this->request->post['reason']) && is_scalar($this->request->post['reason'])
            ? mb_substr(trim(strip_tags((string)$this->request->post['reason'])), 0, 255, 'UTF-8')
            : '';
        ...
        $reservation = $this->model_extension_robokassa_payment_robokassa->reserveRefund(
            $order_id,
        ...
        if (!$result['success']) {
            ...
            $this->model_extension_robokassa_payment_robokassa->failRefundSubmission($reservation['refund_id'], $status, $result['error'], $result['raw']);

            if ($status === 'unknown') {
                $this->model_extension_robokassa_payment_robokassa->addOrderNote($order_id, $this->language->get('note_refund_unknown'));
            }
        ...
        if (empty($response['success']) || !$request_id_valid) {
            ...
            $this->model_extension_robokassa_payment_robokassa->failRefundSubmission($reservation['refund_id'], $status, $message, $result['raw']);
        ...
        $this->model_extension_robokassa_payment_robokassa->completeRefundSubmission($reservation['refund_id'], $request_id, $result['raw']);
        $this->model_extension_robokassa_payment_robokassa->addOrderNote($order_id, sprintf($this->language->get('note_refund_created'), number_format($amount, 2, '.', ''), $request_id, $invoice_items ? '' : ' ' . $this->language->get('text_refund_without_receipt')));
    ...
    {
        if (!$this->user->hasPermission($modify ? 'modify' : 'access', 'extension/robokassa/payment/robokassa')) {
    ...
    private function generateRefundCronToken(): string
    {
        return bin2hex(random_bytes(24));
    }

    private function saveRefundCronToken(string $token): void
    {
    ...
    private function getRefundCronUrl(string $token): string
    {
        return HTTP_CATALOG . 'index.php?route=extension/robokassa/payment/robokassa|refundCron&token=' . rawurlencode($token);
    }

    private function maskRefundCronUrl(string $url): string
    {
        return (string)preg_replace('/([?&]token=)[^&]*/', '$1••••••••••••', $url);
    }

    public function copyRefundCronUrl(): void
    {
        $json = [];
    ...
    public function regenerateRefundCronToken(): void
    {
        $json = [];
        ...
        } elseif (!$this->user->hasPermission('modify', 'extension/robokassa/payment/robokassa')) {
            $this->response->addHeader('HTTP/1.1 403 Forbidden');
    ...
    private function registerEvents(): void
    {
        ...
        $events = [
            ...
            [
                'code'        => 'robokassa_admin_menu_before',
                'description' => 'Robokassa shortcut in admin menu',
                'trigger'     => 'admin/view/common/column_left/before',
                'action'      => 'extension/robokassa/event/robokassa|onAdminMenuBefore',
                'status'      => 1,
                'sort_order'  => 18
            ],
        ];
