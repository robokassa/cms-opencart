...
class ControllerExtensionPaymentRobokassa extends Controller
{
    public function refundCron()
    {
        ...
        $this->load->model('extension/payment/robokassa_refund');

        if (!$this->model_extension_payment_robokassa_refund->acquireLock()) {
        ...
        try {
            ...
            foreach ($this->model_extension_payment_robokassa_refund->getPendingRefunds(50) as $refund) {
                $summary['checked']++;
                $result = $api->getState($refund['request_id']);

                if (!$result['success']) {
                    $this->model_extension_payment_robokassa_refund->updateState($refund['refund_id'], $refund['status'], $result['raw'], $result['error']);
                    $summary['errors']++;
                    continue;
                }
                ...
                if (!in_array($status, array('processing', 'finished', 'canceled'), true)) {
                    $this->model_extension_payment_robokassa_refund->updateState($refund['refund_id'], 'processing', $result['raw'], 'Robokassa returned an unknown refund status.');
                    $summary['errors']++;
                    continue;
                }

                $state_updated = $this->model_extension_payment_robokassa_refund->updateState($refund['refund_id'], $status, $result['raw']);
                $summary[$status]++;
                ...
                if ($state_updated && $status === 'finished') {
                    ...
                    $this->model_extension_payment_robokassa_refund->recordFinished($refund['order_id'], $comment);
                } elseif ($state_updated && $status === 'canceled') {
                    $comment = sprintf('Robokassa: возврат отменён. ID: %s', $refund['request_id']);
                    $this->model_extension_payment_robokassa_refund->recordCanceled($refund['order_id'], $comment);
                }
            }
        } catch (Exception $e) {
        ...
        $this->model_extension_payment_robokassa_refund->releaseLock();
