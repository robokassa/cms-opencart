<?php
class ModelExtensionPaymentRobokassaYandexSplit extends Model {
    ...
    public function getMethod($address, $total) {
        ...
        if ($this->config->get('payment_robokassa_status') && $this->config->get('payment_robokassa_yandex_split_status')) {
            ...
            if (!$this->config->get('payment_robokassa_geo_zone_id') && $total >= 10 && $total <= 200000 && $this->checkCurrency('YandexPaySplit')) {
                $status = true;
            } elseif ($query->num_rows && $total >= 10 && $total <= 200000 && $this->checkCurrency('YandexPaySplit')) {
                $status = true;
            } else {
                $status = false;
            }
        ...
        if ($status) {
            $method_data = array(
                'code'       => 'robokassa_yandex_split',
                'title'      => $this->language->get('text_title'),
                'terms'      => '',
                'sort_order' => $this->config->get('payment_robokassa_yandex_split_sort_order')
            );
        }
